...
- Responses API. The AI creation form has no duration field; the teacher sets an actual time limit in the quiz settings before running the test.
...
- Quality checks reject comma-counting tasks that already show the commas and image-choice tasks that disclose the answer or try to express a non-observable temporal meaning through one image. An independent `gpt-5.6-luna` review checks each generated text question and answer for clear semantic errors and duplicates; flagged questions enter the bounded replacement workflow, then the test is reviewed again (at most three review passes). The same check runs on per-question regeneration. Review failure rejects the draft instead of publishing unreviewed questions. This is still not a guarantee of factual correctness; teachers review drafts before publication. More review passes mean additional tokens, time and API cost.
- Unexpected backend failures are classified into actionable callable errors and logged with a short reference code and processing phase, without logging uploaded material or complete prompts. AI media is prepared before a new quiz document is saved so a failed image request leaves no empty draft. A write failure during question persistence still triggers cleanup of the partially created quiz. Previously created empty drafts cannot safely be distinguished from intentional manual drafts and are not deleted automatically.
...
- The admin Feedback tab filters KI-Aufgaben, shows reason counts and lets the admin inspect the question snapshot and mark negative reports as reviewed. Positive ratings are recorded as completed. On every generation the backend reads red question reports of **all teachers**, without the former 300-record/author-only limit. Prior reports of intrinsically incorrect, ambiguous or answer-revealing questions block similar future questions, regardless of who reported them. Duplicate questions and failed image renders are specific to a test or rendered image, so those reports inform the independent review's error priorities without blocking the question text globally. No identities, teacher comments or archived snapshots are sent to the reviewer or to other teachers; the generated draft itself is reviewed. When a teacher explicitly requests AI replacement, that teacher's note is sent as an instruction for that one replacement. Ratings adjust application checks, not the base model's weights. A full scan of red question feedback uses Firestore reads on every generation, so move to a materialized summary as volumes grow. Review reports and update versioned rules and regression cases for new failure patterns.
- Generated illustrations and image answers are additionally inspected by `gpt-5.6-luna` with image input against the requested scene; on a clear mismatch, one new image is generated and inspected. If both versions fail, the entire new test draft fails and any partial quiz is removed. The stored feedback snapshot does not archive image binaries, uploaded materials or student work. Automated vision can miss mistakes and produce false alarms, so teachers still inspect the rendered images.
...
- Image-answer options use optional `imageDataUrl` / `imageAlt` fields on existing single/multi options; old options remain valid. Visual checking uses extra `gpt-5.6-luna` image input tokens, and retrying a failed image can increase cost.
...
Rights holders can submit notices without a teacher account through a public form; reports stay in the private admin feedback inbox. After verifying a report, an admin can suspend access to a specific quiz and shared template using a Firestore-enforced `rightsHold`. See `RIGHTS_AND_REPORTING.md` for manual triage, monitoring gaps and legal review before commercial use.
