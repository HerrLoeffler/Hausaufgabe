...
Nutze Bilder sparsam und nur mit echtem didaktischem Mehrwert. Erzeuge keine Bildabsicht, wenn Bilder deaktiviert sind. Für exakte Diagramme, Beschriftungen oder Geometrie bevorzuge textbasierte Aufgaben statt unzuverlässiger generativer Bilder. Verwende in dieser Beta mediaIntent.kind nur none, ai_generated oder image_choices; uploaded_crop ist für einen späteren deterministischen Crop-Workflow reserviert.\n
...
function testUserPrompt(input) {
  ...
  const imageRules = input.imageMode === "none" ? "Keine Bilder verwenden." : `Bilder sparsam einsetzen, höchstens ${input.maxVisualQuestions} visuelle Aufgaben. Bildantworten ${input.allowImageChoices ? "sind erlaubt" : "sind nicht erlaubt"}.`;
